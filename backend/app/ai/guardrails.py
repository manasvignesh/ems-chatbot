...
PROMPT_INJECTION_PATTERNS = [
    r"(?i)\bignore\s+(all\s+)?(previous|prior|above|\w+\s+)?(instructions|prompts|rules)\b",
    r"(?i)\bforget\s+(the\s+)?(all\s+|\w+\s+)?(guardrails|instructions|system\s+prompt|rules)\b",
    r"(?i)\b(reveal|show|print|tell|give\s+me|expose)\s+(your\s+)?(system\s+prompt|hidden\s+prompt|instructions|initial\s+prompt)\b",
    r"(?i)\b(reveal|show|print|tell|give\s+me|what\s+is|output)\s+.*?\b(api\s*key|gemini\s*key|supabase.*key|service\s*role.*key|secret\s*keys|service\s*role|token|secret|credentials|password)\b",
    r"(?i)\b(system\s*override|override\s*system)\b",
